...
const emitAndSaveAlert = async (alertData, io) => {
    try {
        // 1. Save the alert to MongoDB (including threatSource and analysis)
        const newAlert = await Alert.create(alertData);
        
        // 2. Broadcast if IO is available
        if (io) {
            io.emit('newAlert', newAlert); 
            console.log(`[ALERT] ${newAlert.type} detected from ${alertData.threatSource?.targetURL || alertData.ip || 'unknown'} and broadcasted.`);
        } else {

...
router.post('/', auth, async (req, res) => {
    try {
        const { title, severity, description, source, targetUrl, type, ip, path, payload, threatSource, analysis, message } = req.body;

        // Map title to message and use provided type or default to 'OTHER'
        const alertType = type || 'OTHER';
        const alertMessage = message || title || description || 'Security Alert';
        ...
        const newAlert = await Alert.create({
            ...
            ip: ip || threatSource?.sourceIP || '0.0.0.0',
            path: path || threatSource?.sourcePath || targetUrl || '/api',
            payload: payload || 'N/A',
            timestamp: new Date(),
